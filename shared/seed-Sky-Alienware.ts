import { addDoc, Timestamp, getDocs, deleteDoc, doc, updateDoc, collection, db } from './firebaseConfig';
...
// Hardcoded sample product names for deletion
...
/**
 * Resets the seeded database data by deleting specific hardcoded sample data.
 * WARNING: This will delete ONLY the seeded sample data, preserving manually created data!
 */
export async function resetDatabase(): Promise<void> {
  try {
    console.log('Resetting seeded database data...');

    // Hardcoded sample user emails to delete
    const sampleUserEmails = sampleUsers.map(user => user.email);
    ...
    for (const auctionDoc of auctionsSnapshot.docs) {
      ...
      // Delete auction (all auctions are seeded)
      await deleteDoc(auctionDoc.ref);
    }
    console.log('Deleted seeded auctions');

    // Delete seeded products by name
    const productsSnapshot = await getDocs(collection(db, 'products'));
    let deletedProducts = 0;
    for (const productDoc of productsSnapshot.docs) {
      if (sampleProductNames.includes(productDoc.data().name)) {
        await deleteDoc(productDoc.ref);
        deletedProducts++;
      }
    }
    console.log(`Deleted ${deletedProducts} seeded products`);

    // Delete seeded users by email EXCEPT super admin
    const usersSnapshot = await getDocs(collection(db, 'users'));
    let deletedUsers = 0;
    for (const userDoc of usersSnapshot.docs) {
      const userData = userDoc.data();
      if (userDoc.id !== SUPER_ADMIN_UID && sampleUserEmails.includes(userData.email)) {
        await deleteDoc(userDoc.ref);
        deletedUsers++;
      }
    }
    console.log(`Deleted ${deletedUsers} seeded users (preserved super admin and manual users)`);

    console.log('Seeded database data reset completed successfully!');
  } catch (error) {
    console.error('Error resetting seeded database data:', error);
    throw error;
  }
}
